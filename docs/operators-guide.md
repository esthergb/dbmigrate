...
- Conflict report JSON includes categorized `failure_type` values (for example `schema_drift`, `conflict_duplicate_key`) and `sql_error_code` when surfaced by the destination engine.
